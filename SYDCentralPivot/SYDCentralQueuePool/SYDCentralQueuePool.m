...
//  SYDServiceSDK
//
//  Created by panzhengwei on 2019/1/10.
...
- (dispatch_queue_t) createSerialQueue:(NSString *) queueTag
{
    ...
    if(!queue)
    {
        ...
        LOG_DEBUGGING_NS(@"create serial queue for [%@] failed, so use the main queue",queueTag);
    }
    else
    {
        LOG_DEBUGGING_NS(@"create serial queue for [%@] successfully",queueTag);
    }
...
- (void) asyncPerformBlock:(void(^)(void)) dispatchBlock inQueueForTag:(const NSString *) queueTag
{
    ...
    if(!queue)
    {
        LOG_DEBUGGING_NS(@"serial queue for [%@] not exist,so use main queue",queueTag);
        queue = dispatch_get_main_queue();
    }
