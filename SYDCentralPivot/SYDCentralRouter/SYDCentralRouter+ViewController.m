...
{
    LOG_DEBUGGING_NS(@"SYDCentralRouter_enterViewController: viewControllerKey[%@] config[%@] paramDic[%@]",viewControllerKey,config,paramDic);
    
    Class viewControllerClass = [self.centralFactory getViewControllerClass:viewControllerKey];
    
    if(!viewControllerClass)
    {
        LOG_DEBUGGING_NS(@"SYDCentralRouter_enterViewController: viewcontrollerclass for [%@] not exist",viewControllerKey);
        return;
    }
    ...
    {
        LOG_DEBUGGING_NS(@"SYDCentralRouter: enterViewController method for [%@] not exist ",viewControllerKey);
    }
}
