...
{
    LOG_DEBUGGING_NS(@"SYDCentralRouter_sendMessageToService: serviceKey[%@] message[%@] paramArray[%@]",serviceKey, message,paramArray);
    
    id serviceBean = [self.centralFactory getSYDServiceBean:serviceKey];
    
    if(!serviceBean)
    {
        LOG_DEBUGGING_NS(@"SYDCentralRouter_sendMessageToService: serviceBean for [%@] not exist",serviceKey);
        return nil;
    }
    ...
    if(isInstanceMessage)
    {
        ...
        {
            LOG_DEBUGGING_NS(@"SYDCentralRouter_sendMessageToService: serviceBean [%@] can not respondsToInstanceSelector[%@]",serviceKey,message);
            return nil;
        }
    ...
    else
    {
        ...
        {
            LOG_DEBUGGING_NS(@"SYDCentralRouter_sendMessageToService: serviceBean [%@] can not respondsToClassSelector[%@]",serviceKey,message);
            return nil;
        }
    ...
    {
        LOG_DEBUGGING_NS(@"SYDCentralRouter_sendMessageToService: async perform message [%@] for service[%@]",message,serviceKey);
    ...
    {
        LOG_DEBUGGING_NS(@"SYDCentralRouter_sendMessageToService: sync perform message [%@] for service[%@]",message,serviceKey);
